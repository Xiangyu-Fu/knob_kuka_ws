...
public:
  ...
  bool init()
  {
    ...
    {
      std::cout << "LWRHWFRI::init(): Did you forget to set the port/ip?" << std::endl << "You must do that before init()" << std::endl << "Exiting..." << std::endl;
      return false;
    }
    ...
    if(!setScheduling(*KRCCommThread_.get()))
    {
      std::cout << "LWRHWFRI::init(): failed to set scheduler, do you have permission?" << std::endl;
      return false;
    }
...
private:
  ...
  bool setScheduling(std::thread& th)
  {
    ...
    if (max_thread_priority != -1)
    {
      ...
      int ret = pthread_setschedparam(th.native_handle(), SCHED_FIFO, &params);
      if(ret != 0)
      {
      ...
      int policy = 0;
      ret = pthread_getschedparam(th.native_handle(), &policy, &params);
      if( ret != 0)
      {
